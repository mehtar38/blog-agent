...
def suggest_topics():
    ...
    # Numbered choice for selecting the topic
    lines = [l.strip() for l in response.text.strip().split("\n") if l.strip()]

    all_titles = [l.split(". ", 1)[-1] for l in lines if l]
    data["suggested_topics"].extend(all_titles) 

    if choice.isdigit() and 1 <= int(choice) <= len(lines):
        selected = all_titles[int(choice) - 1]
    else:
        selected = choice

    data["suggested_topics"] = list(set(data["suggested_topics"]))
    data["current_topic"] = selected
    save_topics(data)
